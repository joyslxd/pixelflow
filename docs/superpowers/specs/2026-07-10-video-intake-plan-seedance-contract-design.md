...
- 用户确认的画幅贯穿场景资产图与全部视频生成接口；视频模型只负责分镜视频，图片模型只负责角色、场景和道具图片。
...
| `video_ratio` | 下拉单选 | 是 | `9:16` | 取视频模型与图片模型共同支持的比例 |
...
- 手动切换视频模型后，按 4.5 节规则与当前图片模型共同计算可选画幅；当前画幅失效时执行兼容性调整。
...
- 图片模型 API 异常时使用最小兜底配置 `gpt-image-2`，并向用户提示配置读取失败。

表单标签必须显示为“图片模型”，避免和“视频模型”混淆。

### 4.5 视频画幅与图片模型兼容性

场景资产图必须和视频保持同一画幅，因此最终可选画幅取以下交集：

```text
selected_video_model.paramConfig.aspectRatioList
∩ selected_image_model.paramConfig.aspectRatioList
∩ PixelFlow 支持画幅 {9:16, 16:9, 1:1}
```

- 交集包含 `9:16` 时默认选择 `9:16`。
- 切换视频模型或图片模型后重新计算交集。
- 当前画幅失效时自动切换到交集中的 `9:16`，否则使用第一个共同画幅，并明确提示用户发生了兼容性调整。
- 两个模型不存在共同画幅时禁用提交，提示用户更换视频模型或图片模型。
- 图片生成清晰度不作为本次表单字段。系统从所选图片模型的 `paramConfig.sizeList` 中优先选择 `4K`，其次选择 `2K`、`1080p`，最后使用第一个可用值，并把解析结果保存到创作合同的 `scene_image_size`。
...
角色三视图、场景图和道具图生成请求必须继承：

- `creation_contract.video_ratio` 作为图片比例。
- `creation_contract.image_model` 作为 content-app 图片生成模型。
- `creation_contract.scene_image_size` 作为图片清晰度。

所选图片模型必须来自 `/api/modelParamConfig/listByCategory/image_generate`。若图片模型不支持该比例，表单阶段应优先阻止提交；后端场景资产 Skill 仍要再次返回可操作错误，不能悄悄使用其他比例或换模型。生成后的图片 URL 继续进入现有全局资产与 `@mention` 体系。
...
- 动态视频模型过滤、视频系统推荐、图片模型默认/LLM 预选和双模型画幅交集测试。
...
- 完成一条视频生成流程，从需求清洗表单选择视频模型和图片模型、创意、Plan、场景包、参考图、分镜视频到合并结果。
