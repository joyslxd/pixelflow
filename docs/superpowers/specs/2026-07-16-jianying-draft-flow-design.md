...
真实剪映草稿第三方接口合同已经到位。当前实现保持既有 PixelFlow 内部边界，并新增 HTTP Provider：异步创建/轮询第三方任务，下载多个草稿 JSON，在服务端打成 ZIP，通过 content-app 上传 TOS 后返回下载地址。
...
- 不在 PixelFlow 内解释或改写第三方返回的草稿 JSON 内容。
...
flowchart LR
    ...
    F --> G["HttpJianyingDraftSkill"]
    G --> H["第三方创建/查询接口"]
    H --> K["多个 JSON URL"]
    K --> L["ZIP + content-app /api/upload"]
    E --> I["异步 Job 与版本幂等"]
...
配置完整时 `/capability` 返回 `available=true`。Provider 未配置时返回 `available=false` 和用户可展示原因；如果调用方绕过前端直接请求 `/start`，后端返回 `not_configured`，不创建空任务。
...
`HttpJianyingDraftSkill` 映射真实第三方合同；`UnavailableJianyingDraftSkill` 只在配置不完整时返回能力未配置。
...
- Provider 配置缺失时，入口仍置灰并展示服务待接入提示。
...
剪映草稿能力配置在开发和生产环境启用，两个环境当前使用同一 Provider 域名：
...
- 使用 Fake Skill 走通内部异步状态机，并用 HTTP Mock 验证创建、轮询、JSON 校验、ZIP 和 TOS 上传。
- 使用真实 Provider 做端到端联调，验证返回的 JSON 能归档为一个可下载 ZIP。
...
2. Provider 配置完整时按钮可用；配置缺失时置灰并提示“剪映草稿服务待接入”。
3. 内部 API、Skill、job、版本和状态恢复边界完整可测试。
4. 同一对话、同一分镜版本不会重复生成草稿。
5. 分镜视频发生变化后不会复用旧草稿。
6. 任务和消息严格写回来源对话。
7. 视频流程结束后仍保留草稿入口。
8. 第三方返回的多个 JSON 被打包成一个 ZIP，经 content-app 上传 TOS 后可从前端下载。
