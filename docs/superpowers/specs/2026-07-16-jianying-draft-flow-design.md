...
PixelFlow 当前视频主流程在全部分镜视频生成并合并成功后，只提供“无意见，结束”和“提出修改意见”。本次在最终视频确认阶段增加“生成剪映草稿”能力，让用户把当前版本的全部分镜视频转换为可在剪映专业版中继续编辑的草稿 ZIP。
...
3. **不传时间裁剪信息**：PixelFlow 内部只传分镜顺序和可访问的视频地址，片段时长由未来 Provider 从视频文件读取。
...
- 分镜必须按顺序规范化后提交。
- 每个分镜必须有 `scene_id` 和可访问的 HTTP/HTTPS `video_url`。
