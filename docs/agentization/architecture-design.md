...
- M00 提供本地检查和远端门禁；每个切片在 commit/push 前执行。机器指令类注释允许进入最小例外清单，其他自动检查无法判断的内容由独立 reviewer 逐项确认。任一项不符合时不得 push、不得进入阶段集成候选。
...
- 认可提交说明、工程记录和新增/修改解释性注释使用中文；新增/修改配置必须逐项提供详细中文说明，并在 M00 后由本地脚本、独立审核和远端门禁共同阻止不合规 push/集成。
