...
>
> 当前结论：四阶段上线方案已确认，当前目标为 R1。模块之间并行、模块内部切片严格串行；M00 首次集成手动启动一次，之后明确阶段检查点和模块最终提交由远端单槽流水线自动集成。每个下一切片、生产灰度比例和真实付费冒烟仍需人工启动/批准。所有实现模块尚未开工。

## 上线里程碑

| 批次 | 目标日 | 用户可见成果 | 当前状态 | 代码检查点 | 生产状态 |
| --- | ---: | --- | --- | --- | --- |
| R1 | D4 | 自动上下文压缩开始/完成提示、输入排队、刷新恢复、原任务继续 | `design_ready` | 待 M00/M01/M03/M04/M07/M12.3/M13.1 | 未发布 |
| R2 | D9 | 视频会话 Agent：继续、修改、重生、重试、新建、切换、取消、追问 | `planned` | 待 M02/M05/M06/M11/M12.5/M13.2 | 未发布 |
| R3 | D13 | 图片/编辑、PPT、视频分析接入同一会话 Agent | `planned` | 待 M08/M09/M10/M13.3 | 未发布 |
| R4 | D16–D18 | 五流程全量门禁、灰度、回滚、新对话全面接管 | `planned` | 待全部模块和 M13.4–M13.5 | 未发布 |
...
| M13 | R1–R4 增量 E2E、Shadow、灰度、回滚 | A+B | `not_started` | 0/5 | 各批次按阶段依赖；最终 M01–M12 | — |
