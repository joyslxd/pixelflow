...
- 硬阻塞：M00-A 无。完整 SHA `0af72ff6993e9e67636f21e8e16d641411702d67` 已按用户批准设计作为门禁启用前精确历史豁免，不改写历史；只放过仍由该提交 blame 拥有的英文行，并显式清除 ignore-revs 配置影响。`M00-I.1` 只验证 M00-A、M00-B 和 M00 范围门禁，不执行 M02 清理，也不执行 M13 后端全量回归。
