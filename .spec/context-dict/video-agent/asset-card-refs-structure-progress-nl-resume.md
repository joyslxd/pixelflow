...
---
## 结论摘要
模型选择卡支持上传参考图 + 自然语言用途说明；`generate-scene-assets` 按资产名/类型绑定参考图，角色也可走 `reference_image`。`prepare-scene-packages` 通过 `structure_progress` 暴露结构子阶段。

V2.1：**删除**前端 `resolveWorkflowResumeIntent`。「继续 / 开始生图 / 继续编辑脚本」等 NL 一律 `turns/start` → 思考流；模型卡确认按钮仍显式开生图（V2 Turn / `frontend_v2` Job）。`frontend_v2` 历史会话仍可本地续查 pending Job。
