...
| `/api/modelParamConfig/listByCategory/video_generate` | `GET` | `web/src/lib/api.ts` 的 `listVideoGenerateModelConfigs()`，由视频需求清洗表单触发 | 查询可用视频模型、画幅和其他模型参数。 | `ModelParamConfigController.listByCategory()` | 前端展示 content-app 返回的所有启用且 `modelType` 包含 `seedance` 的配置；系统推荐优先解析为 `seedance-2.0`，并把实际模型展示给用户。画幅只能从所选模型的 `paramConfig.aspectRatioList` 中选择；清晰度、声音和参考素材等模型特有能力继续以实时配置与生成 API 为准。 |
