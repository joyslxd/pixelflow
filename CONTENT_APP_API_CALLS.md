...
M06视频 Provider 的 start 与 status 都复用同一次浏览器请求的 content-app `Authorization`：Repository 仅保存请求 SHA-256 和 provider job ID；Authorization 只保存在 Gateway 进程内、按 provider job ID 绑定的短期租约中，不写入配置、数据库、Workspace、日志或对话 Snapshot。进程重启或租约到期后，已受理任务保持原状态，用户下一次显式授权后才能继续安全查询；不得以持久化用户 token 的方式绕过该边界。
...
| `/api/task/{taskId}/status` | `GET` | `run_generation.poll_task()`；M06 视频 Provider 使用进程内临时租约 | 轮询异步生成、拆解和 SmartPPT 任务，直到完成、失败或超时。 | `TaskController.getTaskStatus()` | 复用创建该任务的浏览器 Authorization；Gateway 重启或租约到期后必须由用户重新授权，禁止持久化 token。HTTP 200且`success=false,data=null`固定映射为`expired`并要求上层创建新 attempt。 |
