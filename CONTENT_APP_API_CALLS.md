...
M06可恢复Provider例外边界：`backend/pixelflow/skills/borgrise/provider_jobs.py`的start只在单次调用栈透传用户`Authorization`与`Idempotency-Key`，Repository只保存请求SHA-256和provider job ID；重启后的status禁止保存或复用用户token，只接受显式注入的服务端状态查询凭据。测试环境已验证content-app接受服务级Bearer查询既有task状态，Gateway通过运行时环境变量`PIXELFLOW_CONTENT_APP_STATUS_AUTHORIZATION`读取该凭据：用途是为重启恢复提供独立status鉴权；类型必须为`Bearer <token>`字符串；缺失或格式非法会让参考视频、场景视频和QA质检Provider失败关闭，现有R1/v2流程不受影响；值变更由下一次status调用即时读取，无需重启；回滚时删除该变量即可恢复失败关闭。该变量属于敏感服务凭据，只能从部署密钥系统注入，不得写入YAML、源码、日志、Repository或对话Snapshot。
...
| `/api/creative/analyze_image` | `POST` | `run_generation.analyze_image()`，由 `/agent/flows/video/update-scene-package-asset/start` 的 Python job 在全局素材替换后触发 | 分析单张新素材中的人物、物品、场景和视觉特征，供 LLM 定向更新引用该素材的分镜描述。 | `CreativeController.analyzeImage()` | 请求体传 `imageUrl`，请求头传模型 `gemini-3-flash-preview` 和 `bill_type=1`，不传 `projectId`；返回 taskId 后轮询 `/api/task/{taskId}/status`，结果从 `data.result.data.image_analysis_markdown` 读取，默认最多等待 15 分钟。网络或服务异常按统一规则最多重试 3 次，业务失败和额度不足立即停止。图片分析结果只允许用于替换旧素材的外貌、外观、特征和特点，不得改写分镜结构、时间段、故事逻辑或其他素材；精确补丁未通过校验时只允许携带原因定向修复一次；删除素材不调用此接口。 |
| `/api/creative/decompose_video_to_storyboard` | `POST` | `skill._decompose_blocking()`；V2可恢复入口使用`provider_jobs.make_reference_analysis_job_service()` | 将用户上传/输入的参考视频拆解为 storyboard shots，供后续 Brief、分镜规划或视频分析结果展示使用。 | `CreativeController.decomposeVideoToStoryboard()` | 旧入口会在一次调用内轮询；V2入口只start并把taskId交给M06持久化，显式透传`Idempotency-Key`，结果只投影白名单分镜。 |
| `/api/creative/video_quality_review` | `POST` | `run_generation.review_video_quality()`；V2可恢复入口使用`provider_jobs.make_quality_review_job_service()` | 对合并视频和各分镜视频执行 QAAgent QC 质检，返回问题、受影响分镜和修复建议。 | `CreativeController.reviewVideoQuality()` | 旧前端流程仍通过`/agent/flows/video/quality-review/start`包装；V2入口只start并持久化taskId，重启后用服务凭据查询原task。进入Workflow的结果仅投影`passed/summary/quality_report/issues/affected_scene_ids/revision_prompt`白名单且不保留raw，供应商附加字段不得进入Snapshot。 |
| `/api/video/text-to-video` | `POST` | `run_generation.text_to_video()`；V2可恢复入口使用`provider_jobs.make_scene_video_job_service()` | 无参考素材时按镜头 prompt 生成视频。 | `VideoController.textToVideo()` | 请求体精确为 `prompt/model/ratio/size/duration/videoCount/sound`；不传 `projectId`；V2只start并把taskId交给M06，完成后将安全HTTPS视频URL同时投影为镜头版本和权威工作区视频资产。 |
...
| `/api/video/merge` | `POST` | `run_generation.merge_videos()`；V2 live入口使用`provider_jobs.make_merge_video_job_service()` | 按分镜顺序同步下载、合并并上传最终视频。 | 视频合并Controller | 请求体仅传`videoUrls`；头部透传当次用户Authorization、Operation幂等键、模型/计费/时长/清晰度。V2把同步响应直接原子提交为M06终态，不轮询task status；单镜头不调用本接口。 |
| `/api/task/{taskId}/status` | `GET` | `run_generation.poll_task()`；V2可恢复入口使用`provider_jobs.ContentAppTaskJobService.status()` | 轮询异步生成、拆解和 SmartPPT 任务，直到完成、失败或超时。 | `TaskController.getTaskStatus()` | 旧wrapper使用当前请求用户Authorization；M06重启恢复只允许`PIXELFLOW_CONTENT_APP_STATUS_AUTHORIZATION`提供的服务级Bearer，禁止持久化或复用用户token。HTTP 200且`success=false,data=null`固定映射为`expired`并要求上层创建新attempt。 |
...
| `/api/picture/smart-ppt/generatePptContentToJson` | `POST` | `run_generation.generate_ppt_content_json()`，由 `pixelflow_ppt.start_ppt_content_json()` 触发 | 将确认后的 PPT 大纲转为页面 JSON。 | `SmartPptController.generatePptContentToJson()` | 请求 body 传 `originalOutline`、`smartPptProjectId`、`pptStyle`；轮询结果读取 `content_json`。若大纲包含 `## P1` 等显式页标题，调用前将页内 `###` 子标题降级为加粗普通内容，避免供应商误拆页。 |
...
以下接口不属于 content-app。旧v2调用集中在`backend/pixelflow/jianying_draft/http_skill.py`并衔接普通`/api/upload`；R2 live调用集中在`jianying_draft/provider_jobs.py`并衔接`/api/internal/upload`。域名和固定token从profile配置读取。
