...
| 持久化 External Job Coordinator | M06.2 模块分支切片 | 已建立 operation 四段身份、规范请求哈希、显式状态迁移、重复 start claim，以及双 worker 互斥的数据库 lease、heartbeat、`next_poll_at` 和过期接管；只持久化摘要而不保存 Authorization。供应商适配、完成恢复和重启接管仍由 M06.3–M06.5 后续切片实现，本模块增量尚未进入 Agent 长期分支 |
...
│   │   ├── agent_runtime/           # Turn、Snapshot/SSE、上下文预算、摘要压缩、排队与 operation 幂等/租约
