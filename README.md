...
- 场景视频生成 job 内部会并行生成分镜视频，当前最多 3 个分镜同时调用 content-app；所有分镜都结束后再统一判断。全部成功时按 `scene_index` 合并；部分异常时返回 `failed_scenes` 和每个失败原因，重试只提交失败分镜；部分额度不足时整批只提示一次额度不足，充值后同样只重试额度暂停或异常分镜。
