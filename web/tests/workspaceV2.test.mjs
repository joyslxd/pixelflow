...
const { operationCounts, projectWorkspaceV2 } = await import(moduleUrl);
...
test("V2 长片完整保留 17 段与 6+6+5 批次，单个失败不覆盖其它状态", () => {
  const projection = projectWorkspaceV2({
    ...
    operation_batches: [
      { batch_id: "batch-1", status: "polling", children: Array.from({ length: 6 }, (_, index) => ({ operation_id: `a-${index}`, status: "succeeded" })) },
      { batch_id: "batch-2", status: "polling", children: Array.from({ length: 6 }, (_, index) => ({ operation_id: `b-${index}`, status: index === 0 ? "failed" : "polling" })) },
      { batch_id: "batch-3", status: "queued", children: Array.from({ length: 5 }, (_, index) => ({ operation_id: `c-${index}`, status: "queued" })) },
    ],
  });
  ...
  assert.deepEqual(operationCounts(projection.batches), { queued: 5, polling: 5, succeeded: 6, failed: 1, paused: 0 });
