...
import { isNearScrollBottom, pinScrollToBottom } from "@/lib/conversationScroll";
...
export function ConversationMessages({
  ...
  const pinnedConversationRef = useRef<string | undefined>(undefined);

  useLayoutEffect(() => {
    if (pinnedConversationRef.current !== conversationId) {
    ...
    const pinIfNeeded = () => {
      if (stickToBottomRef.current) pinScrollToBottom(scroller);
    };
    pinIfNeeded();
    if (typeof ResizeObserver === "undefined" || content === null) return undefined;
    const observer = new ResizeObserver(pinIfNeeded);
    observer.observe(content);
    return () => observer.disconnect();
  }, [conversationId, loading, messages, processing, responsePreview, executionSummary]);

  return (
    <div
      ref={scrollerRef}
      className="min-h-0 flex-1 overflow-y-auto p-6 [overflow-anchor:none]"
      onScroll={(event) => {
