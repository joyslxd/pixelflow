...
function scopedMessages(messages, sourceMessageId) {
    if (!sourceMessageId)
        return messages;
...
function creationIntent(value) {
    return value === "video" || value === "image" || value === "ppt" ? value : null;
}
function inferIntentFromPhase(phase) {
    if (phase.startsWith("ppt_"))
        return "ppt";
    if (phase.startsWith("scene_") || phase.startsWith("video_"))
        return "video";
...
function inferIntentFromMessages(messages) {
    for (const message of [...messages].reverse()) {
        ...
        if (!artifact)
            continue;
        if (artifact.type === "ppt_outline" || artifact.type === "ppt_images" || artifact.type === "ppt_file")
            return "ppt";
        if (artifact.type === "video_scene_packages" || artifact.type === "video_result" || artifact.type === "video_quality_review")
            return "video";
        if (artifact.type === "image_result" && !artifact.sceneGlobalAssetEditReview)
            return "image";
...
function currentStatusForPhase(phase, currentIndex, lastIndex) {
    if (phase === "form_cancelled")
        return "cancelled";
    if (/quota_paused|blocked/.test(phase))
        return "paused";
    if (/failed/.test(phase))
        return "failed";
    if (currentIndex === lastIndex)
        return "waiting_download";
    if (/running|analyze|generation|merge/.test(phase))
        return "processing";
    return "waiting";
}
function phaseIndexForVideo(phase, scenePackageStage) {
    if (/^(?:message_|intake_)|form_cancelled/.test(phase) || phase.endsWith("_form_pending"))
        return 0;
    if (/direction|creative_directions/.test(phase))
        return 1;
    if (/^plan_|plan_review/.test(phase))
        return 2;
    if (/^scene_global_asset|^scene_asset/.test(phase))
        return 4;
    ...
    if (/^video_(?:generated|regenerated|accepted)$/.test(phase))
        return 6;
    if (/^video_/.test(phase))
        return 5;
    return null;
}
function phaseIndexForPpt(phase) {
    if (/^(?:message_|intake_)|form_cancelled/.test(phase) || phase === "ppt_form_pending")
        return 0;
    if (/^ppt_outline_running|^ppt_outline_failed/.test(phase))
        return 1;
    if (/^ppt_outline/.test(phase))
        return 2;
    if (/^ppt_content_json|^ppt_images|^ppt_image_/.test(phase))
        return 3;
    if (/^ppt_file_ready|^ppt_done/.test(phase))
        return 5;
    if (/^ppt_file|^ppt_job/.test(phase))
        return 4;
    return null;
}
function phaseIndexForImage(phase, directEdit) {
    if (/^(?:message_|intake_)|form_cancelled/.test(phase) || phase.endsWith("_form_pending"))
        return 0;
    if (directEdit && /^image_edit_(?:waiting|options|model)/.test(phase))
        return 0;
    if (/direction|creative_directions/.test(phase))
        return 1;
    if (/^plan_|plan_review|image_generation_blocked|image_prepare/.test(phase))
        return 2;
    if (/^image_(?:generated|regenerated|accepted|edit_done)$/.test(phase))
        return 4;
    if (/^image_/.test(phase))
        return 3;
    return null;
}
function phaseIndex(intent, phase, flowKind, scenePackageStage) {
    if (intent === "video")
        return phaseIndexForVideo(phase, scenePackageStage);
    if (intent === "ppt")
        return phaseIndexForPpt(phase);
    return phaseIndexForImage(phase, flowKind === "direct_image_edit");
}
function latestArtifact(messages, predicate) {
    for (const message of [...messages].reverse()) {
        if (message.artifact && predicate(message.artifact))
            return message.artifact;
    }
    return null;
}
function artifactIndexAndStatus(intent, messages) {
    if (intent === "video") {
        ...
        if (latestArtifact(messages, (artifact) => artifact.type === "plan" && artifact.intent === "video"))
            return { index: 2, status: "waiting" };
        if (latestArtifact(messages, (artifact) => artifact.type === "directions" && artifact.intent === "video"))
            return { index: 1, status: "waiting" };
        return null;
    }
    if (intent === "ppt") {
        const file = latestArtifact(messages, (artifact) => artifact.type === "ppt_file" && Boolean(artifact.pptFile));
        if (file?.pptFile?.ok && file.pptFile.ppt_url)
            return { index: 5, status: file.deliveryDownloadedAt ? "completed" : "waiting_download" };
        if (file?.pptFile && !file.pptFile.ok)
            return { index: 4, status: file.pptFile.quota_insufficient ? "paused" : "failed" };
        const images = latestArtifact(messages, (artifact) => artifact.type === "ppt_images" && Boolean(artifact.pptImages));
        if (images?.pptImages)
            return { index: 3, status: images.pptImages.ok ? "waiting" : images.pptImages.quota_insufficient ? "paused" : "failed" };
        if (latestArtifact(messages, (artifact) => artifact.type === "ppt_outline" && Boolean(artifact.pptSummary)))
            return { index: 2, status: "waiting" };
        return null;
    }
    const result = latestArtifact(messages, (artifact) => artifact.type === "image_result" && Boolean(artifact.imageResult) && !artifact.sceneGlobalAssetEditReview);
    ...
    if (latestArtifact(messages, (artifact) => artifact.type === "plan" && artifact.intent === "image"))
        return { index: 2, status: "waiting" };
    if (latestArtifact(messages, (artifact) => artifact.type === "directions" && artifact.intent === "image"))
        return { index: 1, status: "waiting" };
    return null;
}
export function deriveWorkflowTaskBoard(input) {
    const progress = input.progress || null;
    const phase = String(progress?.last_phase || input.lastPhase || "").trim().toLowerCase();
    if (progress && !progress.intent)
        return null;
    if (phase.startsWith("video_analysis") || phase === "intake_unknown")
        return null;
    ...
    const flowKind = directEdit ? "direct_image_edit" : "standard";
    ...
    const steps = definitions.map((definition, stepIndex) => {
        ...
        if (stepIndex < index)
            return { ...definition, status: "completed" };
        if (stepIndex === index)
            return { ...definition, status: currentStatus };
