...
2. 镜头描述由一个或多个中文段落组成；每个段落必须以当前分镜内部的 `0-4秒` 这类整数秒范围开头，不得使用 ms、毫秒、小数时间码。
3. 内容严格承接当前 plan.md 和创作合同，不擅自改变时长、画幅、模型、商品卖点或转化目标。
4. 每个场景独立可生成；跨场景用相同资产、服装、道具、色彩和空间方向保持连续性。
5. 段落数量由本镜内容决定：只有单一主体完成一个连续动作且景别、运镜、声音和叙事重点没有阶段变化时才使用一个段落；出现动作阶段、景别、运镜、说话者、声音或叙事重点变化时必须另起段落。
6. 多个段落的时间范围必须从 0 秒开始，按顺序无重叠、无缺口地覆盖到本镜结束；每个时间段独占一个段落，不能把多个时间段挤在同一段。
7. 每个段落显式使用“地点：”“主体：”“动作：”“景别：”“运镜：”“光影：”“声音：”“收束：”八个标签；“收束”说明进入下一段、下一镜或全片结束。

每段推荐组织顺序：`时间范围 -> 地点与主体 -> 动作变化 -> 景别与运镜 -> 光影与风格 -> 声音或对白 -> 本段收束或衔接`。

## 参考素材与一致性

- 只使用调用方声明的 `@asset_id`，写成 `@character-x`、`@scene-x`、`@prop-x`。
- 每个分镜最多 9 张图片参考；同一素材不要重复计数。
- 该上限必须在 Plan 策划阶段满足：若某镜需要的具体角色、场景、商品和道具合计超过 9 个，应拆分分镜或重排时长、动作、对白与资产，并把关联叙事一起移动以保持连续；不得先输出超限 Plan，也不得通过截断、漏掉或删除全局资产凑数。
...
- 描述由一个或多个按内容决定的中文段落组成；每段有独立整数秒范围，多段连续覆盖整镜，整镜包含主体、动作、镜头、光影和声音意图。
...
- 用毫秒或小数时间码，或把多次动作、景别/运镜变化和对白变化全部塞进一个 `0-N秒` 段落。
- 多个时间段写在同一段、段落没有独立时间范围，或多段之间存在缺口、重叠和越界。
