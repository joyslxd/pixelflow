"""Tests for the POST /agent/auth/initialize endpoint.
...
def test_initialize_creates_admin_and_sets_cookie(client):
    ...
    resp = client.post("/agent/auth/initialize", json=_init_payload())
...
def test_initialize_needs_setup_false(client):
    ...
    client.post("/agent/auth/initialize", json=_init_payload())
    me = client.get("/agent/auth/me")
...
def test_initialize_register_does_not_block_initialization(client):
    ...
    client.post("/agent/auth/register", json={"email": "regular@example.com", "password": "Tr0ub4dor3a"})
...
def test_setup_status_before_initialization(client):
    ...
    resp = client.get("/agent/auth/setup-status")
...
def test_setup_status_returns_cached_result_on_rapid_calls(client):
    ...
    resp1 = client.get("/agent/auth/setup-status")
    assert resp1.status_code == 200

    # Immediate second call returns cached result, not 429.
    resp2 = client.get("/agent/auth/setup-status")
...
def test_setup_status_does_not_return_stale_true_after_initialize(client):
    ...
    before = client.get("/agent/auth/setup-status")
    assert before.status_code == 200
    assert before.json()["needs_setup"] is True

    init = client.post("/agent/auth/initialize", json=_init_payload())
    assert init.status_code == 201

    after = client.get("/agent/auth/setup-status")
