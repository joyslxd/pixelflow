...
class TestFollowUpAssociation:
    ...
    async def test_follow_up_auto_detection_logic(self):
        ...
        store = MemoryRunStore()
        await store.put("r1", thread_id="t1", status="success", created_at="2026-01-01T00:00:01+00:00")
        await store.put("r2", thread_id="t1", status="error", created_at="2026-01-01T00:00:02+00:00")
        ...
        await store.put("r3", thread_id="t1", status="success", created_at="2026-01-01T00:00:03+00:00")
        recent = await store.list_by_thread("t1", limit=1)
        follow_up = None
        if recent and recent[0].get("status") == "success":
            follow_up = recent[0]["run_id"]
