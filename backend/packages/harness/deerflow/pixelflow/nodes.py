...
async def generate_node(state: TaskState) -> TaskState:
    """生成: shot-by-shot generation via the Borgrise client.

    TODO: expand prompts (PromptEngine) and call BorgriseClient image->video /
    extend-video per shot, polling each task to completion.
    """
    logger.info("[pixelflow] generate task_id=%s", state.get("task_id"))
