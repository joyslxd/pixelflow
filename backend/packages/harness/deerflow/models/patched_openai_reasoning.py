"""OpenAI 兼容网关的 reasoning_content 提取补丁。

标准 ``langchain_openai.ChatOpenAI`` 明确不提取第三方字段
``reasoning_content``（DeepSeek / AIRouter 等）。入场思考流依赖该字段
才能把思维链推到 ``channel=reasoning``；本适配器在流式 chunk 与非流式
结果中把它写入 ``additional_kwargs.reasoning_content``。
"""
...
def _extract_reasoning_content(value: Any) -> str | object:
    """从 dict / Pydantic 对象读取 reasoning_content；保留空字符串。"""
...
class PatchedChatOpenAIReasoning(ChatOpenAI):
    """ChatOpenAI：流式/非流式均保留 OpenAI 兼容网关的 reasoning_content。"""
    ...
    def _convert_chunk_to_generation_chunk(
        ...
        if choices:
            ...
            if reasoning is not _MISSING and isinstance(
