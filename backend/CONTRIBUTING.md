...
所有 Workspace 修改必须携带 `expected_workspace_revision`。计费或异步外部任务必须经 M06 Operation；长期记忆写入必须经 `LongTermMemoryService` 和 WriteOutbox。Authorization 只在请求作用域内使用，不能存入日志、配置、Snapshot、测试 fixture 或 Sidecar。
